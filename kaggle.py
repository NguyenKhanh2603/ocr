...
class MatchingEngine:
    ...
    def find_optimal_matches(self, original_lines: List[str], ocr_lines: List[str]) -> Tuple[
        List[MatchCandidate], Dict]:
    ...
    def _generate_all_candidates(self, original_lines: List[str], ocr_lines: List[str]) -> List[MatchCandidate]:
        ...
        if self.config.debug:
            ...
            for match in candidates:
                print(
                    f"  Match: {match.original_indices} -> {match.ocr_indices}, Error: {match.error_score}, Lengths: {match.original_text_length}, {match.ocr_text_length}",
                    file=sys.stderr)
        ...
        def backtrack(last_end: int, last_secondary_index: int, total_gap: int, accumulated_error: int,
            ...
            for i, match in enumerate(new_matches):
                ...
                if last_end == -1:
                    ...
                    gap = (match.start_pos - last_end - 1) if primary_text[
                                                                  last_end] == ' ' and match.start_pos > last_end else (
                                match.start_pos - last_end)
    ...
    def _evaluate_combination_fast(
        ...
        if not self._meets_error_threshold(ref_text if secondary_line_source == 'original' else primary_text,
                                           final_error):
            return None
    ...
    def _select_optimal_combination(self, candidates: List[MatchCandidate]) -> List[MatchCandidate]:
        ...
        for i, candidate in enumerate(candidates):
            ...
            if orig_overlap or ocr_overlap:
                ...
                print(
                    f"  Error savings: {savings} (penalty: {unmatched_orig_penalty + unmatched_ocr_penalty}, error: {candidate.error_score})",
                    file=sys.stderr)
        ...
        for match in reference_matches:
            ...
            if orig_overlap or ocr_overlap:
                print(
                    f"Reference used target lines: {match.original_indices} -> {match.ocr_indices} (error: {match.error_score})",
                    file=sys.stderr)
    ...
    def _bounded_backtracking_search(self, candidates: List[MatchCandidate],
        ...
        if self.config.debug:
            ...
            for i, (savings, candidate) in enumerate(candidates_with_savings[:5]):
                print(f"  {i + 1}. Savings: {savings:.1f}, Error: {candidate.error_score}, "
                      f"Match: {candidate.original_indices} -> {candidate.ocr_indices} ({candidate.match_type})",
                      file=sys.stderr)
            ...
            if idx == len(candidates):
                if current_total_error < best_total_error:
                    ...
                    if self.config.debug:
                        ...
                        print(
                            f"  New best: {len(current_matches)} matches, match_error: {match_error}, total_error: {current_total_error}",
                            file=sys.stderr)
                return False
        ...
        if self.config.debug:
            print(f"Starting bounded backtracking search with reference total error: {reference_total_error}",
                  file=sys.stderr)
...
def format_output(results: Dict, config: Config) -> str:
    ...
    if config.verbose:
        ...
        for i, match in enumerate(sorted_matches):
            ...
            if len(match.original_indices) > 1: output.append(
                f"    => Combined: \"{' '.join(results['original_lines'][j] for j in match.original_indices)}\"")
            output.append(f"  OCR (Indices: {match.ocr_indices})")
            for j in match.ocr_indices: output.append(f"    L{j:03d}: {results['ocr_lines'][j]}")
            if len(match.ocr_indices) > 1: output.append(
                f"    => Combined: \"{' '.join(results['ocr_lines'][j] for j in match.ocr_indices)}\"")

        output.append("\n--- Unmatched Original Lines (Deletions) ---")
        unmatched_orig = [f"  L{i:03d}: {line} (Penalty: {len(line)})" for i, line in
                          enumerate(results['original_lines']) if i not in results['used_original_indices']]
        output.extend(unmatched_orig if unmatched_orig else ["  None"])

        output.append("\n--- Unmatched OCR Lines (Insertions) ---")
        unmatched_ocr = [f"  L{i:03d}: {line} (Penalty: {len(line)})" for i, line in enumerate(results['ocr_lines']) if
                         i not in results['used_ocr_indices']]
...
def main():
    ...
    norm_group.add_argument("--disable-case-conversion", action="store_false", dest="case_conversion",
                            help="Disable converting text to lowercase.")
    norm_group.add_argument("--disable-unicode-normalization", action="store_false", dest="unicode_normalization",
                            help="Disable Unicode NFC normalization.")
    norm_group.add_argument("--disable-width-conversion", action="store_false", dest="width_conversion",
                            help="Disable converting full-width to half-width characters.")
    norm_group.add_argument("--disable-punctuation-removal", action="store_false", dest="punctuation_removal",
                            help="Disable removal of most punctuation.")
    norm_group.add_argument("--disable-whitespace-normalization", action="store_false", dest="whitespace_normalization",
                            help="Disable normalizing multiple spaces to one and trimming.")
    norm_group.add_argument("--disable-blank-line-removal", action="store_false", dest="blank_line_removal",
                            help="Disable removing blank lines.")
